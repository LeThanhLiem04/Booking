...
const AdminPanel = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      switch (dialogType) {
        case 'user':
          await updateUser(selectedItem._id, formData);
          await fetchUsers();
          break;
        case 'booking':
      ...
      if (response?.data) {
        ...
        toast.success(
          <div>
            ...
            <p>Email notification has been sent.</p>
            <p>Booking details:</p>
            <ul>
              <li>Hotel: {currentBooking.roomId?.hotelId?.name}</li>
              <li>Room: {currentBooking.roomId?.name}</li>
              <li>Check-in: {new Date(currentBooking.checkInDate).toLocaleDateString()}</li>
              <li>Check-out: {new Date(currentBooking.checkOutDate).toLocaleDateString()}</li>
            </ul>
          </div>,
          {
            autoClose: 5000,
          ...
          <Table>
            ...
            <TableBody>
              {loadingReviews ? (
                ...
                <TableRow>
                  <TableCell colSpan={5}>No reviews found.</TableCell>
                </TableRow>
              ) : (
                reviews.map((r) => (
                  <TableRow key={r._id}>
          ...
          <Table>
            ...
            <TableBody>
              {loadingRooms ? (
                ...
                <TableRow>
                  <TableCell colSpan={8}>No rooms found.</TableCell>
                </TableRow>
              ) : (
                rooms.map((r) => (
                  <TableRow key={r._id}>
